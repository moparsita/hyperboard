...
<section className="container m-auto py-5">
    ...
    <div className="rounded-large shadow-lg my-1 inline-block p-4 relative w-full bg-white">
        ...
        <div className="grid grid-cols-2 font-bold px-5 py-2">
            ...
            <p className=""><small>مساحت: </small> {ad.side} مترمربع</p>
            <p className=""><small>نوع نورپردازی: </small> {ad.lightType.name}</p>
        </div>

    </div>
    <hr className="my-3"/>
    <div className="rounded-large shadow-lg my-1 inline-block p-4 relative w-full bg-white">
        <h3 className="block text-lg font-bold flex flex-row align-middle items-center "><IconSax.People color="#9818D6" size="36" className="ml-2"/>  اطلاعات فنی تابلو</h3>
        ...
        <div className="grid grid-cols-2 font-bold px-5 py-2">
        <p className="flex flex-row"><small>شرایط بیننده: </small> <img src={ad.viewCondition.icon.thumb} width="30" alt="{ad.viewCondition.name}"/> {ad.viewCondition.name}</p>
        <p className="flex flex-row"><small>نوع بیننده: </small> <img src={ad.viewerType.icon.thumb} width="30" alt="{ad.viewerType.name}"/> {ad.viewerType.name}</p>
        <p className=""><small>جنسیت بیننده: </small> {ad.viewerGender} </p>
        <p className=""><small>مدت زمان رویت: </small> {ad.viewDuration}  ثانیه</p>
        <p className=""><small>سرعت هنگام رویت: </small> {ad.viewSpeed}  متر بر ثانیه</p>
        <p className=""><small>اوقات مناسب: </small> {ad.dayTime.name}</p>
        </div>
    </div>
</section>
